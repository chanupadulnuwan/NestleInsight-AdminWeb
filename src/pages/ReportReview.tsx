...
function SummaryCard({ label, value }: { label: string; value: string | number | null | undefined }) {
  return (
    <div className="rounded-[1.2rem] border border-[#eee2d7] bg-[#fff9f5] px-4 py-4">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#a37d63]">{label}</p>
      <p className="mt-1.5 text-[1.1rem] font-bold text-[#4d3020]">{value ?? '—'}</p>
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-[1.5rem] border border-[#ebdfd5] bg-white p-6 shadow-[0_32px_80px_rgba(59,31,15,0.18)]">
        <h3 className="text-[1.05rem] font-bold tracking-[-0.02em] text-[#4d3020]">{title}</h3>
...
export default function ReportReview() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      setData(await fetchDailyReportDetail(id!))
    } catch (e) {
  ...
  const closePopup = () => { setActivePopup(null); setPopupReason('') }
  ...
  const routeSummary = report.routeSummaryJson as Record<string, unknown> | null
  const visitSummary = report.visitSummaryJson as Record<string, unknown> | null
  const osaSummary = report.osaSummaryJson as Record<string, unknown> | null
  const deliverySummary = report.deliverySummaryJson as Record<string, unknown> | null
  const incidentSummary = report.incidentSummaryJson as Record<string, unknown> | null
    ...
    <>
      ...
      <div className="min-h-screen bg-[#fdf8f4] px-4 py-6 sm:px-6 sm:py-8">
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className={surfaceClassName + ' px-6 py-6 sm:px-7'}>
            ...
            {report.repComments && (
              <div className="mt-5 rounded-[1.1rem] border border-[#eee2d7] bg-[#fff9f5] px-4 py-4">
                ...
                <p className="mt-2 text-sm leading-6 text-[#4d3020]">{report.repComments}</p>
              </div>
            )}
          </div>
          ...
          {routeSummary && (
            <div className={surfaceClassName + ' px-6 py-6 sm:px-7'}>
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#a37d63]">Route Summary</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <SummaryCard label="Field Duration" value={`${routeSummary.fieldDurationMinutes ?? 0} min`} />
                <SummaryCard label="Opening Stock Lines" value={String(routeSummary.openingStockLines ?? 0)} />
                <SummaryCard label="Closing Stock Lines" value={String(routeSummary.closingStockLines ?? 0)} />
                <SummaryCard label="Return Lines" value={String(routeSummary.returnLineCount ?? 0)} />
              </div>
            </div>
          )}
          ...
          {visitSummary && (
            <div className={surfaceClassName + ' px-6 py-6 sm:px-7'}>
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#a37d63]">Visit Summary</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <SummaryCard label="Total Visits" value={String(visitSummary.totalVisits ?? 0)} />
                <SummaryCard label="Completed" value={String(visitSummary.completedVisits ?? 0)} />
                <SummaryCard label="Total Duration" value={`${visitSummary.totalDurationMinutes ?? 0} min`} />
                <SummaryCard label="Outlet Feedback" value={String(visitSummary.feedbackCount ?? 0)} />
              </div>
            </div>
          )}
          ...
          {osaSummary && (
            <div className={surfaceClassName + ' px-6 py-6 sm:px-7'}>
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#a37d63]">OSA Summary</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <SummaryCard label="Planogram OK" value={String(osaSummary.visitsWithPlanogramOk ?? 0)} />
                <SummaryCard label="POSM OK" value={String(osaSummary.visitsWithPosmOk ?? 0)} />
                <SummaryCard label="OSA Issues" value={String(osaSummary.issueCount ?? 0)} />
                <SummaryCard label="Outlets with Issues" value={String(osaSummary.outletCountWithIssues ?? 0)} />
              </div>
              {Array.isArray(osaSummary.issues) && (osaSummary.issues as unknown[]).length > 0 && (
                <div className="mt-4 overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#eee2d7]">
                        <th className="pb-2 text-left text-xs font-semibold text-[#8a6c58]">Outlet</th>
                        <th className="pb-2 text-left text-xs font-semibold text-[#8a6c58]">Issue type</th>
                        <th className="pb-2 text-left text-xs font-semibold text-[#8a6c58]">Note</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(osaSummary.issues as Record<string, unknown>[]).slice(0, 10).map((issue, i) => (
                        <tr key={i} className="border-b border-[#f5ede6]">
                          <td className="py-2 text-[#4d3020]">{String(issue.outletName ?? issue.shopName ?? '—')}</td>
                          <td className="py-2 text-[#7f6657]">{String(issue.issueType ?? '—')}</td>
                          <td className="py-2 text-[#7f6657]">{String(issue.note ?? '—')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {deliverySummary && (
            <div className={surfaceClassName + ' px-6 py-6 sm:px-7'}>
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#a37d63]">Delivery Summary</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                <SummaryCard label="Assisted Orders" value={String(deliverySummary.assistedOrderCount ?? 0)} />
                <SummaryCard label="Total Order Value" value={`LKR ${Number(deliverySummary.totalOrderValue ?? 0).toLocaleString()}`} />
              </div>
            </div>
          )}

          {/* Incident Summary */}
          {incidentSummary && Number(incidentSummary.incidentCount ?? 0) > 0 && (
            <div className={surfaceClassName + ' px-6 py-6 sm:px-7'}>
              <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#a37d63]">Incidents</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <SummaryCard label="Total Incidents" value={String(incidentSummary.incidentCount ?? 0)} />
              </div>
              {Array.isArray(incidentSummary.incidents) && (incidentSummary.incidents as unknown[]).length > 0 && (
                <div className="mt-4 overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#eee2d7]">
                        <th className="pb-2 text-left text-xs font-semibold text-[#8a6c58]">Type</th>
                        <th className="pb-2 text-left text-xs font-semibold text-[#8a6c58]">Severity</th>
                        <th className="pb-2 text-left text-xs font-semibold text-[#8a6c58]">Description</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(incidentSummary.incidents as Record<string, unknown>[]).map((inc, i) => (
                        <tr key={i} className="border-b border-[#f5ede6]">
                          <td className="py-2 text-[#4d3020]">{String(inc.incidentType ?? inc.type ?? '—')}</td>
                          <td className="py-2 text-[#7f6657]">{String(inc.severity ?? '—')}</td>
                          <td className="py-2 text-[#7f6657]">{String(inc.description ?? '—')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
